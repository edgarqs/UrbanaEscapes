    ...
    <form method="GET" action="{{ route('reservas.checkins') }}">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="date" name="data_sortida" id="data_sortida"
                    class="form-control @error('data_sortida') is-invalid @enderror" value="{{ $dataSortida }}">
            </div>
            <div class="form-group">
        ...
        <tbody>
            @foreach ($reservas as $reserva)
                        <tr>
                            ...
                            <td>
                                @php
                                    $reservaHoy = $reserva->data_entrada->format('Y-m-d') === \Carbon\Carbon::today()->format('Y-m-d');
                                @endphp
                                @if ($reservaHoy)
                                    <form action="{{ route('reservas.checkins', ['id' => $reserva->id]) }}" method="POST">
                                        @csrf
