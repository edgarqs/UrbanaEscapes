<template>
    <div>
        ...
        <div class="container mx-auto p-4">
            <div v-for="(habitacions, hotelId) in groupedHabitacions" :key="hotelId" class="mb-8 p-4 border border-gray-300 rounded-lg bg-gray-50 max-w-6xl mx-auto">
                <div class="mb-4">
                    <h2 class="text-2xl font-bold text-gray-800">{{ habitacions[0].hotel.nom }}</h2>
                    <p class="text-sm text-gray-600">{{ habitacions[0].hotel.adreca }}, {{ habitacions[0].hotel.ciutat }}, {{ habitacions[0].hotel.pais }}</p>
                </div>
                <div class="w-full flex flex-wrap gap-6">
                    <div v-for="habitacio in habitacions" :key="habitacio.id" class="w-full flex bg-white shadow-lg rounded-lg overflow-hidden">
                        <div class="flex-shrink-0 w-1/3 h-56 overflow-hidden">
                            <!-- Cambiar la imagen basada en el tipo de habitación -->
                            <img :src="getImageForRoomType(habitacio.tipus)"
                                 :alt="`Habitación ${habitacio.id}`" class="w-full h-full object-cover transition-transform transform hover:scale-110">
                        </div>
                        <div class="p-4 flex flex-col justify-between w-2/3">
                            <h5 class="text-xl font-semibold">Habitació {{ habitacio.tipus }}</h5>
                            <p class="text-sm text-gray-700"><strong>Descripció: </strong>Habitació {{ habitacio.tipus }} amb {{ habitacio.llits }} llits y {{ habitacio.llits_supletoris }} llits supletoris.<span v-if="habitacio.id % 2 === 0"> Balconada amb vistes exteriors.</span></p>
                            <p class="text-sm text-gray-700"><strong>Capacitat:</strong> {{ habitacio.llits + habitacio.llits_supletoris }} persones</p>
            ...
            <div id="error-message" class="alert alert-danger hidden bg-red-100 text-red-700 p-4 rounded-lg mt-4" role="alert">
...
export default {
    data() {
        return {
            ...
            roomImages: {
                ...
                "Adaptada": [
                    "https://i.postimg.cc/tRZw4rnw/adaptada1.jpg"
                ]
            }
        };
    },
    ...
    methods: {
                ...
                .then(reserves => {
                    ...
                    const filteredHabitacions = this.habitacions.filter(habitacio => {
                        ...
                        const matchesDestination =
                            this.matchDestination(habitacio, destination);
        ...
        matchDestination(habitacio, destination) {
            ...
            const hotelDetails = [
                habitacio.hotel.adreca,
                habitacio.hotel.ciutat,
                habitacio.hotel.pais
            ].map(detail => detail.toLowerCase()).join(" ");
